...
// Extend the Window interface to include StelWebEngine
declare global {
  interface Window {
    StelWebEngine?: any;
  }
...
export default function Stellarium() {
  ...
  useEffect(() => {
    ...
    script.src = "/stellarium-web-engine/stellarium-web-engine.js";
    script.async = true;
    ...
    script.onload = () => {
      ...
      window.StelWebEngine({
        wasmFile: "/stellarium-web-engine/stellarium-web-engine.wasm",
  ...
  const getObjectDetails = (obj: any) => {
    ...
    try {
      ...
      if (designations && designations.length > 1) {
        const altNames: string = designations.slice(1, 3).map((d: string) => d.replace(/^NAME /, "")).join(", ");
        if (altNames) {
